...
Requires:	libfko => 3.0.0, libpcap, iptables, qrencode
...
if command -v fwknopd >/dev/null 2>&1; then
    fwknopd --key-gen > /tmp/fwknop.keys

TIME_NOW=$(date '+%Y-%m-%d %H:%M:%S')
KEY_LINE="# Auto-generated by RPM install on $TIME_NOW\n$(grep ^KEY_BASE64 /tmp/fwknop.keys | sed 's/^KEY_BASE64:[[:space:]]*/KEY_BASE64 /')"
HMAC_LINE="# Auto-generated by RPM install on $TIME_NOW\n$(grep ^HMAC_KEY_BASE64 /tmp/fwknop.keys | sed 's/^HMAC_KEY_BASE64:[[:space:]]*/HMAC_KEY_BASE64 /')"
if grep -q '^KEY_BASE64' /etc/fwknop/access.conf; then
    sed -i "s|^KEY_BASE64[[:space:]].*|$KEY_LINE|" /etc/fwknop/access.conf
else
    echo "$KEY_LINE" >> /etc/fwknop/access.conf
fi
if grep -q '^HMAC_KEY_BASE64' /etc/fwknop/access.conf; then
    sed -i "s|^HMAC_KEY_BASE64[[:space:]].*|$HMAC_LINE|" /etc/fwknop/access.conf
else
    echo "$HMAC_LINE" >> /etc/fwknop/access.conf
fi

rm -f /tmp/fwknop.keys
